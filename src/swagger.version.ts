...
export function SwaggerVersions(app: INestApplication<any>) {
  ...
  app.getHttpAdapter().get('/api/docs/spec', (req: Request, res) => {
    const version = (req as any).query.v;
    if (version === '2') return res.json(v2Document);
  ...
  SwaggerModule.setup('api/docs', app, v1Document, {
    ...
    swaggerOptions: {
      urls: [
        { url: '/api/docs/spec?v=1', name: 'version 1' },
        { url: '/api/docs/spec?v=2', name: 'version 2' },
      ],
    },
  });
}
